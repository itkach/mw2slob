...
** Installation
   ...
   - [[https://github.com/itkach/slob/][Slob]] (not published on PyPI - installed straight from GitHub, see below)
   ...
   - [[https://gitlab.pyicu.org/main/pyicu][PyICU]] (via Slob), which needs the ICU library itself
   ...
   On Ubuntu/Debian:
   #+BEGIN_SRC sh
   sudo apt-get install libxml2-dev libxslt1-dev libicu-dev pkg-config
   #+END_SRC
   ...
   #+END_SRC
   Homebrew's ~icu4c~ is keg-only (not linked onto ~PATH~), so
...
*** With [[https://docs.astral.sh/uv/][uv]] (recommended)
    ...
    [[https://docs.astral.sh/uv/][uv]] resolves and builds the whole dependency graph in one step,
    ...
    uv run mw2slob --help
    #+END_SRC
    (~uv sync~ creates ~.venv~ in the project directory; ~uv run~
    ...
    default branch:
    #+BEGIN_SRC sh
    uvx --from git+https://github.com/itkach/mw2slob mw2slob --help
    #+END_SRC

    To run a specific released version instead, name its tag in the
    URL:
    #+BEGIN_SRC sh
    uvx --from git+https://github.com/itkach/mw2slob@1.2 mw2slob --help
    #+END_SRC

    If ~mw2slob --version~ ever looks out of date right after a new
...
*** With plain ~pip~
    ...
    Create a virtual environment, then install ~mw2slob~ and ~slob~
